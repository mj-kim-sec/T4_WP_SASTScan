(function ($) {
	...
	$('body').on('click', '.rx-edit-video-source-control, .rx-set-edit-video-url, .rx-edit-review-video-uploader, .rx-manual-review-pro, .rx-coupon-pro', function (e) {
		e.preventDefault();
		var premium_content = document.createElement("p");
		var premium_anchor = document.createElement("a");
	...
	*/
	...
	/* -----------------------------------
			Export multi step form End
		-----------------------------------
	*/


	/* -----------------------------------
			coupon page admin js start here
		-----------------------------------
	*/

	const rxCouponFrom = $(".rx-coupon-form");
	const rxCouponSettingTab = $(".rx-coupon-tab1");
	const rxCouponTab = $(".rx-coupon-tab2");
	const rxCouponEmailTab = $(".rx-coupon-tab3");
	const rxCouponSettingNext = $(".rx-coupon-page1-next");
	const rxCouponPrev = $(".rx-coupon-page2-prev");
	const rxCouponNext = $(".rx-coupon-page2-next");
	const rxCouponEmailPrev = $(".rx-coupon-page3-prev");
	const rxCouponEmailNext = $(".rx-coupon-page3-next");

	const rxCouponSettingPage = $(".rx-coupon-page1");
	const rxCouponPage = $(".rx-coupon-page2");
	const rxCouponEmailPage = $(".rx-coupon-page3");

	$(rxCouponSettingTab).click(function () {
		rxCouponSettingPage.show();
		rxCouponPage.hide();
		rxCouponEmailPage.hide();
	});

	$(rxCouponPrev).click(function () {
		rxCouponSettingPage.show();
		rxCouponPage.hide();
		rxCouponEmailPage.hide();
		rxCouponTab.removeClass('active');
		rxCouponEmailTab.removeClass('active');
	});

	$( rxCouponTab ).click(function () {
		rxCouponSettingPage.hide();
		rxCouponPage.show();
		rxCouponEmailPage.hide();
	});

	$( rxCouponEmailPrev ).click(function () {
		rxCouponSettingPage.hide();
		rxCouponPage.show();
		rxCouponEmailPage.hide();
		rxCouponEmailTab.removeClass('active');
	...
	$( rxCouponSettingNext ).click(function () {
		rxCouponSettingPage.hide();
		rxCouponPage.show();
		rxCouponEmailPage.hide();
		rxCouponTab.addClass('active');
	});

	$(rxCouponEmailTab).click(function () {
		rxCouponSettingPage.hide();
		rxCouponPage.hide();
		rxCouponEmailPage.show();
	});

	$(rxCouponNext).click(function () {
		rxCouponSettingPage.hide();
		rxCouponPage.hide();
		rxCouponEmailPage.show();
		rxCouponEmailTab.addClass('active');
	});

	if ( $(".rx-review-for-discount").is(":checked") ) {
		$(".rx-coupon-enable-field").show();
	}
	// show hide setting field by on off start
	$(".rx-review-for-discount").click(function() {
	    if($(this).is(":checked")) {
	        $(".rx-coupon-enable-field").show(300);
	    } else {
	        $(".rx-coupon-enable-field").hide(200);
	    }
	});

	if ( ! $(".rx-coupon-end-date-check").is(":checked") ) {
		$(".rx-coupon-end-date-container").hide();
	}

	$(".rx-coupon-end-date-check").click(function() {
	    if($(this).is(":checked")) {
	        $(".rx-coupon-end-date-container").show(300);
	    } else {
	        $(".rx-coupon-end-date-container").hide(200);
	    }
	});

	if (!$(".rx_coupon_use_limit_multiple").is(":checked")) {
		$('.rx_coupon_use_limit_value').attr('disabled', true);
	}

	$(".rx_coupon_use_limit_multiple").click(function() {
	    if($(this).is(":checked")) {
	        $('.rx_coupon_use_limit_value').attr('disabled', false);
	    } else {
	        $('.rx_coupon_use_limit_value').attr('disabled', true);
	    }
	});

	let rxCouponRequirnmentValue = $('input[type=radio][name=rx_coupon_minimum_requinment]:checked').val();	
	if( ! rxCouponRequirnmentValue ) {
		$('.rx-coupon-minimum-purchase').attr('disabled', true);
	}	
	if ( rxCouponRequirnmentValue == 'rx_coupon_no_requinment') {
		$('.rx-coupon-minimum-purchase').attr('disabled', true);
		$('.rx-coupon-minimum-quantity').attr('disabled', true);
	}
	if ( rxCouponRequirnmentValue == 'rx_coupon_purchase_requinment') {
		$('.rx-coupon-minimum-purchase').attr('disabled', false);
		$('.rx-coupon-minimum-quantity').attr('disabled', true);
	}
	if ( rxCouponRequirnmentValue == 'rx_coupon_quentity_requinment') {
		$('.rx-coupon-minimum-purchase').attr('disabled', true);
		$('.rx-coupon-minimum-quantity').attr('disabled', false);
	}

	$('input[type=radio][name=rx_coupon_minimum_requinment]').change(function() {
		if (this.value == 'rx_coupon_no_requinment') {
			$('.rx-coupon-minimum-purchase').attr('disabled', true);
			$('.rx-coupon-minimum-quantity').attr('disabled', true);
		}
		if (this.value == 'rx_coupon_purchase_requinment') {
			$('.rx-coupon-minimum-purchase').attr('disabled', false);
			$('.rx-coupon-minimum-quantity').attr('disabled', true);
		}
		if (this.value == 'rx_coupon_quentity_requinment') {
			$('.rx-coupon-minimum-purchase').attr('disabled', true);
			$('.rx-coupon-minimum-quantity').attr('disabled', false);
		}
	});
	// show hide setting field by on off end
	$('.rx-coupon-applies-cat-value, .rx-coupon-applies-product-value').select2();
	// select2 multiple product and catagory end

	//select field for coupon start here
	$(".rx-coupon-type").change(function(){
        $(this).find("option:selected").each(function(){
            var optionValue = $(this).attr("value");

            if(optionValue == 'generate_new'){
                $('.rx-coupon-select-fld').hide(300);
                $('.rx-coupon-generate-new-fld').show(300);
            }
			if(optionValue == 'existing_coupon'){
                $('.rx-coupon-select-fld').show(300);
                $('.rx-coupon-generate-new-fld').hide(300);
            }
        });
    }).change();
	//select field for coupon end here

	rxCouponFrom.on("submit", (e) => {
		e.preventDefault();
		var inputObj = {};
		var allObj = {};

		$("#rx-coupon-form input[type='checkbox']:checked, #rx-coupon-form select, #rx-coupon-form input[type='date'], #rx-coupon-form input[type='text'], #rx-coupon-form input[type='number'], #rx-coupon-form input[type='radio']:checked").each(function () {
			inputObj[$(this).attr("name")] = $(this).val();
			inputObj['email_content'] = tinyMCE.activeEditor.getContent();
		});

		$("#rx-coupon-form input[type='checkbox'], #rx-coupon-form select, #rx-coupon-form input[type='date'], #rx-coupon-form input[type='text'], #rx-coupon-form input[type='number'], #rx-coupon-form input[type='radio']").each(function () {
			allObj[$(this).attr("name")] = $(this).val();
			allObj['email_content'] = tinyMCE.activeEditor.getContent();
		});
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'rx_coupon_from_submit',
				rx_selected_coupon_from_data: inputObj,
				rx_all_coupon_data: allObj

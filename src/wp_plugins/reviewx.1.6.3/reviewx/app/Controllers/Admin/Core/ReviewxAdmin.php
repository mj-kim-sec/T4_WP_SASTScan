    ...
    class ReviewxAdmin extends Controller
    {
        ...
        */
        ...
        public function rx_export_review() {
            ...
            $filter_value           = isset( $_POST['filterValue'] ) ? (array) $_POST['filterValue'] : array();
            $filter_field           = array_map( 'esc_attr', $filter_value );
            $selected_column_value  = isset( $_POST['selectedColumns'] ) ? (array) $_POST['selectedColumns'] : array();
            $selected_column        = array_map( 'esc_attr', $selected_column_value );
            $change_column_value    = isset( $_POST['changeColumnName'] ) ? (array) $_POST['changeColumnName'] : array();
            $change_column          = array_map( 'esc_attr', $change_column_value );
            ...
            $selected_rating    = isset( $filter_field[5] ) ? (string)$filter_field[5] : '';
            $review_status      = isset( $filter_field[6] ) ? (string)$filter_field[6] : '';
            $review_sort        = isset( $filter_field[7] ) ? (string)$filter_field[7] : '';
            $review_sort_by     = isset( $filter_field[8] ) ? (string)$filter_field[8] : '';

            $first_commnet_date = json_decode( json_encode($wpdb->get_results( $wpdb->prepare("SELECT 
            ...
            $sql_column_name = implode( ',', $comment_table_columns );
                ...
                if( $product_name == 'all' ){
                    ...
                    $sql_query = $wpdb->prepare(
                        "SELECT $sql_column_name FROM {$wpdb->prefix}comments AS comments
                        ...
                        ",
                    $review_type, $review_status, $start_date, $end_date
                        );
                } else {
                    $review_type = 'review';
                    $sql_query = $wpdb->prepare(
                        "SELECT $sql_column_name FROM {$wpdb->prefix}comments AS comments
                            ...
                            ORDER BY $review_sort $review_sort_by LIMIT $skip_export_row, $export_limit
                            ",
                            $review_type, $review_status, $start_date, $end_date, $product_name
                            );
                }

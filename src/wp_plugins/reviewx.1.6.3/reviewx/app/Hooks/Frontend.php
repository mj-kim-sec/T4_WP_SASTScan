...
$app->addAction(
    'parse_request',
    function() {
        $actual_link = esc_url((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
        $getUri = get_option('_rx_option_unsubscribe_url');
        if ( ! empty($getUri) && is_numeric(strpos($actual_link, $getUri))) {

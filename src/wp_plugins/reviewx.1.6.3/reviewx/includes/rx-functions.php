...
function rx_touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0, $comment_date = null ) {
	...
	echo '</div><input type="hidden" id="ss" name="ss" value="' . esc_html($ss) . '" />';
	...
	foreach ( $map as $timeunit => $value ) {
		...
		echo '<input type="hidden" id="hidden_' . esc_html($timeunit) . '" name="hidden_' . esc_html($timeunit) . '" value="' . esc_html($unit) . '" />' . "\n";
		$cur_timeunit = 'cur_' . $timeunit;
		echo '<input type="hidden" id="' . esc_html($cur_timeunit) . '" name="' . esc_html($cur_timeunit) . '" value="' . esc_html($curr) . '" />' . "\n";
	}
	?>
...
function rx_builder_wc_order_status(){
    ...
    $rx_option_completed = get_option('_rx_option_completed');
    foreach( $statuses as $key=>$value ) {

        $builder_data['type'] = 'checkbox';
        $builder_data['label'] = $value; 
        if( ($key == "completed" && $rx_option_completed === false) || ( $key == "completed" && $rx_option_completed == '1' ) ){
...
function rx_autologin_authenticate() {
  ...
  if (isset($_GET[REVIEWX_AUTOLOGIN_VALUE_NAME])) {    
    ...
    if($autologin_code) {
        $protocol = esc_url((isset($_SERVER["HTTPS"]) && ($_SERVER["HTTPS"] === "on")) ? "https" : "http");

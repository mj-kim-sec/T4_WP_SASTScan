...
abstract class Model {
	...
	private static function should_recreate_tables() {
		// ? Define global $$lasso_recreate_table_time if not existed.
		if ( ! array_key_exists( 'lasso_recreate_table_time', $GLOBALS ) ) {
			global $lasso_recreate_table_time;
			$lasso_recreate_table_time = 0;
		} else {
			global $lasso_recreate_table_time;
		}
	...
	protected function modify_table( $sql, $table, $reference_charset_table = null ) {
		...
		if ( 'Table already exists.' === $result ) {
			...
			if ( 'OK' === $check_msg_txt ) {
				...
				$repair         = self::get_row( "REPAIR TABLE $table" );

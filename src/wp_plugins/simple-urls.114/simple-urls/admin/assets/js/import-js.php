...
<script>
	...
	function init_lasso_view () {
		...
		jQuery(document).ready(function() {
			...
			var search_term = "<?php echo $_GET['search'] ?? ''; // phpcs:ignore ?>";
			var tab_filter = "<?php echo $_GET['filter'] ?? ''; // phpcs:ignore ?>";
			...
			function get_data_via_ajax(page, limit, order_by = undefined, order_type = undefined, container_name = '') {
				...
				jQuery.ajax({
					...
					data: {
						...
						search: search_term,
						keyword: '<?php echo $_GET['keyword'] ?? ''; // phpcs:ignore ?>',
						filter: tab_filter,
				...
				.always(function(res) {
					var page = 'data' in res ? res.data.page : 1;

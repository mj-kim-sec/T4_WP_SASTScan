...
jQuery(document).ready(function () {
    modal_show_import_locations = new lasso_lite_helper.lasso_generate_modal();
    ...
    modal_import_all_notification = new lasso_lite_helper.lasso_generate_modal();
...
function view_location_triggered() {
    jQuery('.show-locations').on('click', function(e) {
        ...
        let table_locations_current_page = lasso_lite_helper.get_pagination_cache(import_locations_local_store_key, lasso_id);
        ...
        get_import_locations( modal_el, lasso_id, table_locations_current_page, false,function (res) {
            ...
            lasso_lite_helper.generate_paging( jQuery('.import-locations-pagination'), res.page, res.count, function (page_number) {
                get_import_locations( modal_el, lasso_id, page_number, true, function (res) {
                    modal_show_import_locations.set_description(build_import_location_modal_content(res.datas), false);
                } );
            }, lasso_lite_helper.link_location_limit);
        });
    });
}
...
function get_import_locations( wrapper_el, lasso_id, page_number = 1, is_loading = false, callback = null ) {
    var data_post = {
        action: 'lasso_lite_get_import_locations',
        lasso_id: lasso_id,
        page_number: page_number,
        limit: lasso_lite_helper.link_location_limit
    };
    jQuery.ajax({
        ...
        .done(function (res) {
            res = res.data;
            lasso_lite_helper.set_pagination_cache(import_locations_local_store_key, res.page, lasso_id);
...
function click_on_import_all_btn() {
    jQuery('.js-import-all-button').unbind().click(function() {
        ...
        jQuery.ajax({
            ...
            type: 'post',
            data: {
                action: 'lasso_lite_import_all_links',
            ...
            beforeSend: function (xhr) {
                ...
                setTimeout(function() {
                    lasso_lite_helper.setProgress(10);
                }, 200);
            }
        })
        .done(function(res) {
            res = res.data;
            if(!res.status) {
                lasso_lite_helper.do_notification('Import failed.', 'red');
            }
        })
...
function click_on_revert_all_btn() {
    jQuery('.js-revert-all-button').unbind().click(function() {
        ...
        jQuery.ajax({
            ...
            type: 'post',
            data: {
                action: 'lasso_lite_revert_all_links',
                filter_plugin: filter_plugin,
            },
            beforeSend: function (xhr) {
                lasso_update_popup.modal('show');
        ...
        .done(function(res) {
            res = res.data;
            if(!res.status) {
                lasso_lite_helper.do_notification('Revert failed.', 'red');
            }
        })
        .fail(function (xhr, status, error) {
            if(xhr.lasso_error) {
...
function click_on_import_btn() {
    jQuery('.js-import-button').unbind().click(function() {
        ...
        lasso_lite_helper.add_loading_button( import_btn );
        ...
        jQuery.ajax({
            ...
            type: 'post',
            data: {
                action: 'lasso_lite_import_single_link',
                import_id: import_id,
...
function click_on_revert_btn() {
    jQuery('.js-revert-button').unbind().click(function() {
        ...
        lasso_lite_helper.add_loading_button(jQuery(this));
        ...
        jQuery.ajax({
            ...
            type: 'post',
            data: {
                action: 'lasso_lite_revert_single_link',
                import_id: import_id,
...
function click_on_bulk_import_btn() {
    jQuery('#btn-bulk-import').unbind().on('click', function () {
        ...
        jQuery.ajax({
            ...
            type: 'get',
            data: {
                action: 'lasso_lite_is_import_all_processing',
            },
            beforeSend: function() {
                lasso_lite_helper.add_loading_button(bulk_import_btn, 'Checking');
            }
        })
        .done(function(res) {
            res = res.data;

	...
	template: `
				<div class="uListing-header-menu" ref="headerMenu" :class="{sticky: sticky}">
					...
					<div class="uhm-actions">
						<button style="padding: 11px 17px;" v-if="showHeader" class="uListing-button uListing-button-text uListing-normalize icon" @click.prevent="saveSettings({url: getAjaxUrl})">{{ getButtonsText.save_changes }}</button>
					</div>
				</div>
	`
}
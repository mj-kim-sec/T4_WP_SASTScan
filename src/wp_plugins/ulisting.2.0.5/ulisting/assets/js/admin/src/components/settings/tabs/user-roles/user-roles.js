	...
	methods: {
		...
		agenciesSwitcher() {
			let mode = toggleValidator(this.agency.value)
				? 'install'
				: 'uninstall'
			postRequest(this.getAjaxUrl, { action: 'stm_agencies_switcher', mode }, res => {
				if (res.success) {
		...
		saveRoles() {
			...
			postRequest(this.getAjaxUrl, {action: 'stm_save_user_roles', roles}, res => {
				if ( res.success ) {
					renderToast(res.message, res.status)
				}
			})
	...
	template: `
			<div v-if="open">
				...
				<div class="accordion-container user-roles">
					<div class="accordion-item" :class="{'uListing-open': edit_panel_id == index, 'uListing-close': edit_panel_id != index}" v-for="(role, index) in roles">
						<div class="accordion-row title">
							<div class="accordion-col">
								<h3 class="uListing-normalize uListing-header-3">{{ role.name }}</h3>
							</div>
							<div class="accordion-col">
								<h4 class="uListing-normalize uListing-header-4">{{ text_domains && text_domains.listing_limit }}: <strong>{{ role.capabilities.listing_limit ? role.capabilities.listing_limit : 0 }}</strong></h4>
							</div>
							<div class="accordion-col" style="margin-right: 0; flex: 1 1 34%">
								<div class="accordion-actions">
									 <span v-if="role.capabilities && (+role.capabilities.default) === 1" class="uListing-info-status active">{{ text_domains && text_domains.default }}</span>
									 <button class="uListing-button uListing-button-text uListing-normalize trash small" :class="{disabled: getActiveStatus !== 1}"  @click.prevent="delete_role(index)">{{ text_domains && text_domains.delete }}</button>
						...
						<template v-if="edit_panel_id == index">
							<div class="accordion-row">
								<div class="accordion-col">
									<div class="uListing-input-field">
										...
										<input type="text" v-model="role.name" class="uListing-input uListing-input-text uListing-normalize input-field medium">
									</div>
								</div>
								<div class="accordion-col">
									<div class="uListing-input-field">
										<span class="uListing-admin-field-title">{{ text_domains && text_domains.listing_limit }}</span>
										<input type="number" v-model="role.capabilities.listing_limit" class="uListing-input uListing-input-text uListing-normalize input-field medium">
									</div>
								</div>
								<div class="accordion-col">
									<div class="uListing-input-field">
										<span class="uListing-admin-field-title">{{ text_domains && text_domains.slug }}</span>
										<input type="text" v-model="role.slug" :class="{disabled: role.slug === 'agency'}" class="uListing-input uListing-input-text uListing-normalize input-field medium">
									</div>
								</div>
							</div>
							<div class="accordion-row" style="margin-bottom: 25px">
								<div class="accordion-col">
									<div class="uListing-checkbox-field">
										<input type="checkbox" :id="'listing_moderation_' + index" value="1" v-model="role.capabilities.listing_moderation">
								...
								<div class="accordion-col" >
									<div class="uListing-checkbox-field">
										<input type="checkbox" :id="'default_' + index" value="1" v-model="role.capabilities.default" @change="(e) => changeDefault(e, index)">
							...
							<div class="accordion-row custom-fields">
								...
								<div class="uListing-draggable-container" :class="{empty: role.custom_fields && role.custom_fields.length <= 0}">
								   <draggable v-model="role.custom_fields" :options="{group:'attribute',  animation: 200, disabled: false, handle:'.handle'}">
										<div class="uListing-draggable-items" v-for="(element, element_index) in role.custom_fields" :key="element_index">
											<div class="uListing-draggable-panel-items-top">
												<div class="title">
													<span class="handle">
														<i class="icon--3"></i>
													</span>
													<p class="uListing-default-text">{{ element.name && element.name.trim().length > 0 ? element.name : 'None' }} </p>
												</div>
												...
												<div class="draggable-row">
													<div class="draggable-col">
														<div class="uListing-admin-select">
															<span class="uListing-admin-field-title">{{ text_domains && text_domains.field_type }}</span>
															<select class="uListing-select-box uListing-select-box-text uListing-normalize medium" v-model="element.type">
																<option v-for="option in type_list" :key="option.id" :value="option.id">{{ option.text }}</option>
															</select>
														</div>
													</div>
													<div class="draggable-col">
														<div class="uListing-input-field">
															<span class="uListing-admin-field-title">{{ text_domains && text_domains.name }}</span>
															<input type="text" v-model="element.name" placeholder="Name" class="uListing-input uListing-input-text uListing-normalize input-field medium" >
														</div>
													</div>
													<div class="draggable-col" style="flex: 1 1 29%">
														<div class="uListing-input-field">
															<span class="uListing-admin-field-title">{{ text_domains && text_domains.slug }}</span>
															<input type="text" v-model="element.slug" placeholder="Slug" class="uListing-input uListing-input-text uListing-normalize input-field medium" >
														</div>
													</div>
												</div>
											</div>
										...
										<div style="display: flex; justify-content: center; align-items: center; margin: 0">
											<button class="uListing-button uListing-button-text icon green with-icon" :class="{disabled: getActiveStatus !== 1}" @click="add_custom_fields(index)">
					...
					<div  style="display: flex;justify-content: space-between; align-items: center">
						<pro-wrapper v-if="getActiveStatus !== 1" type="with-icon green" :btn="text_domains.add_role" text="User Role Add-on." icon="icon-992651"></pro-wrapper>
						<button v-else class="uListing-button uListing-button-text icon green with-icon" @click="add" style="margin-right: 10px; padding-top: 13px; padding-bottom: 13px">
							<i class="icon-992651"></i>

...
import getRequest      from '@plugins/postRequest'
...
export default {
	...
	methods: {
		init() {
			getRequest(this.getAjaxUrl, {action: 'uListing_inventory_page_data', layout_id: this.layout_id}, response => {
				if ( response.success ) {
		...
		remove (id) {
			...
			if ( confirm( 'Are you sure to delete this layout?' ) )
				postRequest(url, {action, id}, response => {
					renderToast(response.message, response.status)
					if ( response.success ) {

...
class StmListingSingleLayout {
    ...
    public static function uListing_active_single_template() {
        $result = [
            'message' => __('Permission denied', 'ulisting'),
            'success' => false,
            'status'  => 'error',
        ];
	...
	public static function get_layout_delete() {
        ...
        if ( current_user_can('manage_options') && isset($_POST['id']) AND $listing_type = StmListingType::find_one(sanitize_text_field($_POST['id']))){
...
//            StmVerifyNonce::verifyNonce(sanitize_text_field($_POST['nonce']), 'ulisting-ajax-nonce');
    ...
    public static function uListing_save_single_layout() {
        ...
        $layout_id = isset($request_data['layout_id']) ? (int)sanitize_text_field($request_data['layout_id']) : null;

...
abstract class CMA_BaseController
{
    ...
    /**
     * Sample Pages and Questions
     */
    protected static $_samplePagesData = array(
        'cm-answers-sample-page' => array(
            'post_type' => 'page',
            'title' => 'CM Answers Sample Page',
            'slug' => 'cm-answers-sample-page',
            'content' => '<p>This is a sample answer page with cminds_answers shortcode, which you can edit! :)</p><p>[cminds_answers can-add-question="y"]</p>',
        ),
        'my-first-question' => array(
            'post_type' => CMA_AnswerThread::POST_TYPE,
            'title' => 'My First Question', 
            'slug' => 'my-first-question',
            'content' => '<p>This is a question!</p><p>The admin can choose if questions should be moderated or automatically approved.</p><p>Then, other users can add answers and engage!</p>',
            'comments' => array(
                array('content' => 'My first answer!', 'approved' => 1),
                array('content' => 'My second answer!', 'approved' => 1),
            ),
        ),
        'what-is-cm-answers' => array(
            'post_type' => CMA_AnswerThread::POST_TYPE,
            'title' => 'What is CM Answers?', 
            'slug' => 'what-is-cm-answers',
            'content' => '<p>The CM Questions and Answers Plugin allows the admin and customers to ask questions, add answers and comments to each question. Both admin and users can rate answers by voting for the posted questions and answers.</p>',
        ),
        'how-to-use-cm-answers-free' => array(
            'post_type' => CMA_AnswerThread::POST_TYPE,
            'title' => 'How To Use CM Answers Free?', 
            'slug' => 'how-to-use-cm-answers-free',
            'content' => '<p>When the plugin is activated it automatically creates a page at (your.domain)/answers/ where your users can add questions and answers.</p><p>Read the tutorial: <a href="https://creativeminds.helpscoutdocs.com/article/2231-cm-answers-cma-getting-started-free-version-guide" target="_blank" rel="noopener nofollow noreferrer">CM Answers (CMA) - Getting Started - Free Version Guide</a>.</p>',
        ),
        'my-resolved-question' => array(
            'post_type' => CMA_AnswerThread::POST_TYPE,
            'title' => 'My Resolved Question', 
            'slug' => 'my-resolved-question',
            'is_resolved' => true,
            'content' => '<p>This question will be marked as resolved. Note that [RESOLVED] is added to its title.</p>',
            'comments' => array(
                array('content' => 'This answer is marked as "resolved" and will close the thread to additional answers.', 'approved' => 1),
            ),
        ),
    );
...
//         	$submenu[apply_filters('CMA_admin_parent_menu', 'options-general.php')][999] = array('Yearly membership offer', 'manage_options', self::PAGE_YEARLY_OFFER);
    ...
    public static function getSamplePagesData() {
        return self::$_samplePagesData;
    }

    /*
    * Generating and removing sample pages
    */
    public static function getSamplePage($samplePageData){
        $args = array(
            'post_type' => $samplePageData['post_type'],
            'name' => $samplePageData['slug'],
    ...
    public static function generateSamplePage($samplePageData){
        if( self::getSamplePage($samplePageData) ){
            self::$_messages[self::MESSAGE_ERROR][] = "Sample Answer Page exists.";
            return false;
        }else{
            $args = array(
                'post_type'   => $samplePageData['post_type'],
                'post_title'  => $samplePageData['title'],
                'post_name'   => $samplePageData['slug'],
                'post_content' => $samplePageData['content'],
                'post_author' => get_current_user_id(),
                'post_status' => 'publish',
            );
            $id = wp_insert_post( $args );
            if (is_wp_error($id)) {
                self::$_messages[self::MESSAGE_ERROR][] = $id->get_error_message();
                return false;
            }else{
                self::$_messages[self::MESSAGE_SUCCESS][] = "Sample Answer Page created succesfully!";

                if($samplePageData['post_type'] == CMA_AnswerThread::POST_TYPE){
                    $thread = CMA_AnswerThread::getInstance($id);
                    if( isset($samplePageData["comments"]) && $samplePageData["comments"] ){
                        foreach ($samplePageData["comments"] as $comment) {
                            $thread->addCommentToThread($comment['content'], get_current_user_id(), false, $comment['approved']);
                        }
                    }
                    if( isset($samplePageData["is_resolved"]) && $samplePageData["is_resolved"] ){
                        $thread->setResolved();
                    }
                }
                return true;
    ...
    public static function tryRemoveSamplePage($samplePageData){
        $_post = self::getSamplePage($samplePageData);
        if(!$_post){
            return false;
        }else{
            $res = wp_delete_post( $_post->ID, true );
            return $res ? true : false;
        }
    }

    public static function generateSamplePages(){
        $samplePagesData = self::getSamplePagesData();
        foreach ($samplePagesData as $samplePageData) {
            self::generateSamplePage($samplePageData);
        }
    }

    public static function tryRemoveSamplePages(){
        $samplePagesData =self::getSamplePagesData();
        foreach ($samplePagesData as $samplePageData) {
            self::tryRemoveSamplePage($samplePageData);
        }
    }

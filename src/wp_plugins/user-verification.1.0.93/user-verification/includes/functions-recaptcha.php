...
//add_filter('wp_authenticate_user', 'user_verification_validate_wc_login_captcha', 10, 2);
function user_verification_validate_wc_login_captcha($user, $password)
{
    $return_value = $user;
    ...
    if ($wc_login_form == 'yes' && isset($_POST['g-recaptcha-response'])) :
        $captcha = isset($_POST['g-recaptcha-response']) ? sanitize_text_field($_POST['g-recaptcha-response']) : '';
...
function wooc_validate_extra_register_fields($username, $email, $validation_errors)
{
    ...
    $captcha_error = isset($user_verification_settings['messages']['captcha_error']) ? $user_verification_settings['messages']['captcha_error'] : __('Captcha Error. Please try again.', 'user-verification');
    $secretkey = isset($user_verification_settings['recaptcha']['secretkey']) ? $user_verification_settings['recaptcha']['secretkey'] : '';
    $res = isset($_POST['g-recaptcha-response']) ? $_POST['g-recaptcha-response'] : '';
...
add_action('lostpassword_post', 'uv_wc_lostpassword_form_validate');
...
function uv_wc_lostpassword_form_validate($validation_errors)
{
    ...
    if ($wc_lostpassword_form == 'yes' && isset($_POST['g-recaptcha-response'])) :

...
if (!class_exists('cdmProjects')) {
    ...
    class cdmProjects
    {
        ...
        function add()
        {
			...
			echo '<div class="cdm-folders-wrapper">
              <div class="cdm-folders-half-left">
...
$selected = $parent[0]['uid'];
  ...
  <tr>
    ...
    <div class="cdm-folders-half-right">';

  do_action('sp_cdm/admin/projects/right_panel');
		...
		$html = '';
		  $spacer = '';  
			...
			$total_levels = $this->get_parent_count($r[$i]['id']);
				...
				$spacer .= '<span style="margin-right:10px">&nbsp;&nbsp; </div>';
				$alpha = $alpha - .1;
				}
                  $html .= '	<tr style="background-color:rgba(221, 221, 221,'.$alpha.')" >
<td colspan="2">'.$spacer.'<span class="sp-caret"></span> #' . stripslashes($r[$i]['id']) . ' ' . stripslashes($r[$i]['name']) . '</td>
				

<td><em>Parent: '.$this->getParentName($r[$i]['parent']).'</em></td>
...
<td></td>
</tr>'.$this->getChildren($r[$i]['id'],	$level ).'';
...
<th style="width:60px"><strong>' . __("ID", "sp-cdm") . '</strong></th>
...
echo '</tr>
                ...
                for ($i = 0; $i < count($r); $i++) {
                    ...
                    echo '	<tr style="background-color:#dddddd;border-bottom:2px solid #CCC">

<td style="font-weight:bold;"><span class="sp-caret"></span> #' . $r[$i]['projectID'] . '</td>				

<td style="font-weight:bold;">' . stripslashes($r[$i]['projectName']) . '</td>

<td style="font-weight:bold;">' . $r[$i]['user_nicename'] . '</td>
...
echo '</tr>';
echo''.$this->getChildren($r[$i]['projectID'] ).'</td>';
                } //$i = 0; $i < count($r); $i++
                echo '</tbody></table>';
            }
        }

...
function cdm_is_premium_active(){
...
';
	...
	$content .=''. $extra_top_menus.'
            ...
            if (cdm_is_premium_active() != 1 && get_option("sp_cdm_ignore") != 1) {
                $content .= '	
...
<a href="http://smartypantsplugins.com/sp-client-document-manager/" target="_blank" class="button">Click here to upgrade! </a> <a style="margin-left:10px" href="http://www.youtube.com/watch?feature=player_embedded&v=m6szdA3r-1Q" target="_blank" class="button">View the youtube video</a> <a style="margin-left:10px" href="http://smartypantsplugins.com/donate/" target="_blank" class="button">Click here to donate</a> <a href="admin.php?page=sp-client-document-manager&sphidemessage=1"  class="button" style="margin-left:10px">Click here to ignore us!</a></p>
    ...
    if (!function_exists('sp_client_upload_admin')) {
        function sp_client_upload_admin()
        {
            ...
            if(cdm_var('s') != '' ){
			$search .=	" AND  (name like '%".cdm_var('s')."%' or file like '%".cdm_var('s')."%' or tags like '%".cdm_var('s')."%'  )";
			}
            
            $search = apply_filters('sp_cdm/admin/search_files/query',$search);
            ...
            if ($user_id != '' && $user_id != '-1') {
                ...
                $query = "SELECT *  FROM " . $wpdb->prefix . "sp_cu   where  parent = 0 ".$search." and recycle = 0 order by id desc LIMIT ".$limit."";
...
<strong>Search Term</strong><br> <input type="text" name="s" value="'. sanitize_text_field(cdm_var('s')).'">
</td>
<td >
<strong>User</strong><br> 
'.wp_dropdown_users(array('name' => 'user_id', 'show_option_none' => 'Choose a user', 'selected'=>sanitize_text_field(cdm_var('user_id')),'echo'=>false)).'
</td>
'.apply_filters('sp_cdm/admin/search_files/fields','').'
<td style="width:70px">
...
#add_action('admin_footer', 'sp_client_upload_admin');
?>
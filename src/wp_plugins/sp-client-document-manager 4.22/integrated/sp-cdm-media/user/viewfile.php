<?php 
$sp_cdm_media_viewfile = new sp_cdm_media_viewfile;
...
add_filter('spcdm/files/responsive/file_toolbox/download_link', array($sp_cdm_media_viewfile, 'download_link'),10,2);
add_filter('spcdm/files/responsive/file_toolbox/preview_link', array($sp_cdm_media_viewfile, 'download_link'),10,2);
add_filter('spcdm/viewfile/edit/upload_field', array($sp_cdm_media_viewfile, 'edit_file'),10,2);
add_action('cdm/view_file/ajax/save' , array($sp_cdm_media_viewfile, 'edit_file_save'),10,2);
add_action('sp_download_file_after_query', array($sp_cdm_media_viewfile, 'share_file'),10,2);
#add_filter('sp_cdm_viewfile_replace_file_infos', array($sp_cdm_media_viewfile, 'file_info_view'),8,4);
class sp_cdm_media_viewfile{
	   
        function share_file($r){
            
            if($r[0]['file'] == 'embed'){
                
             
                echo stripslashes($r[0]['embed']);
                die();
            }
        }
        function edit_file_save($file_id,$update){
            
            global $wpdb;
            
             if(isset($_POST['embed'])){
                 $update_embed = array();
                 $update_embed['embed'] = $_POST['embed'];
                 $where['id'] =  $_POST['fid'];
                 $wpdb->update( $wpdb->prefix . "sp_cu", $update_embed,$where);
        ...
        function edit_file($field,$r){
            
             if($r['file'] == 'embed'){
             return '<p id="cdm-edit-file-upload">
             <textarea name="embed" style="width:100%;height:200px">'.stripslashes($r['embed']).'</textarea>
             
             </p>';   
            }
            return $field;
        }
        function download_link($link,$r){
  ...
  Your browser does not support the video tag.
	...
	function whole_view($html,$r){
		...
		if($style == 'whole' && (in_array($ext,array('mp4','ogg','webm','mp3')) or $r[0]['file'] == 'embed')){
			...
			$html =	 $this->video($stream,$ext,$r[0]['embed']);

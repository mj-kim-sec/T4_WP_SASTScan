<?php
class bookingpress_notifications {
	...
	function bookingpress_notification_dynamic_on_load_methods_func() {
		?>
				this.bookingpress_select_email_notification('Appointment Approved', 'appointment_approved')
	...
	function bookingpress_notification_dynamic_data_fields_func() {
		...
		echo json_encode( $bookingpress_notification_vue_methods_data );
	}
	...
	function bookingpress_notification_dynamic_vue_methods_func() {
		...
		?>
				...
				bookingpress_get_all_default_notification_status(){
					const vm = this;
					let postData = { action: 'bookingpress_get_default_notification_status' };
				...
				bookingpress_get_notification_data(email_notification_key, is_custom_notification = 0, bookingpress_custom_notification_id = 0){
					const vm = this
					...
					bookingpress_get_notification_post_data.bookingpress_notification_name = vm.bookingpress_email_notification_edit_text
					...
					bookingpress_get_notification_post_data._wpnonce = '<?php echo wp_create_nonce( 'bpa_wp_nonce' ); ?>'
				...
				bookingpress_select_email_notification(email_notification_type, email_notification_key, is_custom_notification = 0, bookingpress_custom_notification_id = 0)
				{
					const vm = this
					...
					vm.bookingpress_get_notification_data(email_notification_key)
				...
				bookingpress_save_email_notification_data()
				{
					...
					bookingpress_save_notification_data._wpnonce = '<?php echo wp_create_nonce( 'bpa_wp_nonce' ); ?>'
				...
				bookingpress_insert_placeholder(event)
				{
					const vm = this
					...
					vm.$notify({ title: '<?php esc_html_e( 'Success', 'bookingpress-appointment-booking' ); ?>', message: '<?php echo esc_html( $bookingpress_placeholder_success_msg ); ?>', type: 'success', customClass: 'success_notification',duration:<?php echo intval($bookingpress_notification_duration); ?>,});
				},
				bookingpress_change_tab(){
					const vm = this
					vm.bookingpress_get_notification_data(vm.bookingpress_active_email_notification)
				},				
			<?php
	...
	function bookingpress_get_custom_notification( $bookingpress_notification_id = 0 ) {
		...
		if ( $bookingpress_notification_id != 0 ) {
			$bookingpress_get_custom_notification_data = $wpdb->get_row( "SELECT * FROM {$tbl_bookingpress_notifications} WHERE bookingpress_notification_id = {$bookingpress_notification_id}", ARRAY_A );
			...
			if ( ! empty( $bookingpress_get_custom_notification_data ) ) {
				...
				if ( $bookingpress_custom_notification_type == 'appointment' ) {
					  ...
					  $bookingpress_fetch_selected_services = $wpdb->get_results( "SELECT * FROM {$tbl_bookingpress_notification_services} WHERE bookingpress_notification_id = {$bookingpress_notification_id}", ARRAY_A );
					...
					foreach ( $bookingpress_fetch_selected_services as $bookingpress_fetch_selected_service_key => $bookingpress_fetch_selected_service_val ) {
					  ...
					  $bookingpress_get_custom_notification_data['selected_appointments'] = $bookingpress_selected_services_arr;
					   ...
					   $bookingpress_fetch_selected_events = $wpdb->get_results( "SELECT * FROM {$tbl_bookingpress_notification_events} WHERE bookingpress_notification_id = {$bookingpress_notification_id}", ARRAY_A );
			...
			$bookingpress_get_custom_notification_data = $wpdb->get_results( "SELECT * FROM {$tbl_bookingpress_notifications} WHERE bookingpress_notification_is_custom = 1", ARRAY_A );
			...
			if ( ! empty( $bookingpress_get_custom_notification_data ) ) {
				foreach ( $bookingpress_get_custom_notification_data as $bookingpress_get_custom_notification_key => $bookingpress_get_custom_notification_val ) {
					...
					$bookingpress_get_custom_notification_other_details = $wpdb->get_row( "SELECT * FROM {$tbl_bookingpress_notifications} WHERE bookingpress_notification_id = {$bookingpress_notification_id}", ARRAY_A );
	...
	function bookingpress_save_email_notification_data() {
		...
		if ( ! $bpa_verify_nonce_flag ) {
			...
			$bookingpress_return_data['msg']     = esc_html__( 'Sorry, Your request can not process due to security reason.', 'bookingpress-appointment-booking' );
		...
		if ( ! empty( $_REQUEST ) ) {
			...
			$bookingpress_global_options = $bookingpress_global_options->bookingpress_global_options();

			$bookingpress_allow_tag = json_decode( $bookingpress_global_options['allowed_html'], true );
			...
			$bookingpress_if_notification_exists = $wpdb->get_var( "SELECT COUNT(bookingpress_notification_id) FROM {$tbl_bookingpress_notifications} {$bookingpress_where_condition}" );
			...
			if ( $bookingpress_if_notification_exists > 0 ) {
				...
				$bookingpress_exist_record_data = $wpdb->get_row( "SELECT * FROM {$tbl_bookingpress_notifications} {$bookingpress_where_condition}", ARRAY_A );
	...
	function bookingpress_get_all_default_notification_status() {
		...
		echo json_encode( $bookingpress_default_notification_status_data );
		exit();
	}
}

	global $bookingpress_notifications, $bookingpress_notification_vue_methods_data;
